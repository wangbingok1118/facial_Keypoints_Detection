...
def weight_variable(shape, sdev=0.1):
    ...
    initial = tf.truncated_normal(shape, stddev=sdev, seed=util.SEED)
...
def eval_in_batches(data, sess, eval_prediction, eval_data_node):
    ...
    if size < util.EVAL_BATCH_SIZE:
        raise ValueError("batch size for evals larger than dataset size: %d" % size)

    predictions = np.ndarray(shape=(size, util.NUM_LABELS), dtype=np.float32)
    for begin in range(0, size, util.EVAL_BATCH_SIZE):
        end = begin + util.EVAL_BATCH_SIZE
        ...
        if end <= size:
            ...
            batch_predictions = sess.run(
                eval_prediction,
                feed_dict={eval_data_node: data[-util.EVAL_BATCH_SIZE:, ...]})
...
if __name__ == '__main__':
    ...
    util.dprint("Loading training set from training.csv...")
    train_data, train_labels = util.load(os.path.join(originalDataPath,'training.csv'))
    util.dprint("Loading test set from test.csv")
    test_data, _ = util.load(os.path.join(originalDataPath,'test.csv'))

    util.dprint("Creating validation data.")
    validation_data = train_data[:util.VALIDATION_SIZE, ...]
    validation_labels = train_labels[:util.VALIDATION_SIZE]
    train_data = train_data[util.VALIDATION_SIZE:, ...]
    train_labels = train_labels[util.VALIDATION_SIZE:]

    train_size = train_labels.shape[0]
    util.dprint("%d training samples." % train_size)

    util.dprint("Constructing tensorflow models...")
    train_data_node = tf.placeholder(
        tf.float32,
        shape=(util.BATCH_SIZE, util.IMAGE_SIZE, util.IMAGE_SIZE, util.NUM_CHANNELS))
    # input node to training CNN
    train_labels_node = tf.placeholder(tf.float32, shape=(util.BATCH_SIZE, util.NUM_LABELS))
    ...
    conv1_weights = weight_variable([5, 5, util.NUM_CHANNELS, 32])
    ...
    fc1_weights = weight_variable([util.IMAGE_SIZE // 4 * util.IMAGE_SIZE // 4 * 64, 512])
    ...
    # output layer
    fc3_weights = weight_variable([512, util.NUM_LABELS])
    fc3_biases = bias_variable([util.NUM_LABELS])
    ...
    def model(data, train=False):
        ...
        if train:
            hidden = tf.nn.dropout(hidden, util.DROPOUT_RATE, seed=util.SEED)
    ...
    loss += util.L2_REG_PARAM * regularizers
    ...
    learning_rate = tf.train.exponential_decay(
        util.LEARNING_BASE_RATE,
        global_step * util.BATCH_SIZE,  # current image index in dataset
        train_size,  # decay step
        util.LEARNING_DECAY_RATE,
        staircase=True)

    train_step = tf.train.AdamOptimizer(learning_rate, util.ADAM_REG_PARAM).minimize(loss, global_step=global_step)

    # start tensorflow session
    util.dprint("Starting tensorflow session...")
    ...
    util.dprint("Started training session.")
    while current_epoch < util.NUM_EPOCHS:
        ...
        for step in range(train_size // util.BATCH_SIZE):
            offset = step * util.BATCH_SIZE
            # get batch slice for the step
            batch_data = train_data[offset:(offset + util.BATCH_SIZE), ...]
            batch_labels = train_labels[offset:(offset + util.BATCH_SIZE)]
        ...
        util.dprint("Epoch %04d, train loss %.8f, validation loss %.8f, train/validation %0.8f, learning rate %0.8f" %
        ...
        elif best_valid_epoch + util.EARLY_STOP_PATIENCE < current_epoch:
            util.dprint("Early stopping.")
            util.dprint("Best valid loss was {:.6f} at epoch {}.".format(best_valid, best_valid_epoch))
            break

        current_epoch += 1

    util.dprint("training finished")
    end_time = time.gmtime()

    util.dprint(time.strftime('%H:%M:%S', start_time))
    util.dprint(time.strftime('%H:%M:%S', end_time))

    util.dprint("Generating test results.")
    util.generate_test_results(test_data, sess, eval_prediction, eval_data_node)
