...
def eval_model_batchs(data=None,sess=None): # debug ***
    eval_x_node = data
    eval_prediction = cnnModel(data=eval_x_node,train=False)
    ...
    with tf.Session() as sess:
        ...
        while current_epoch < config.train_epcohs:
            ...
            for batch_index in range(trainDataX.shape[0] // config.mini_batch_size):
                ...
                pass
            print("Epoch %d, train loss %.8f, validate loss " % (current_epoch, train_loss_result))
            validata_predict = eval_model_batchs(data=eval_x_node,sess=sess)
            validate_error = error_measure(predictions=validata_predict,labels=valiDataY)
            print("Epoch %d, train loss %.8f, validate loss %.8f"%(current_epoch,train_loss,validate_error))
            current_epoch += 1
        pass
    pass
